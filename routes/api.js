									...
									UserInfo.findOneAndUpdate({'id':napthe.uid}, {$inc:{red:nhan}}, function(err2, user) {
										if (!!user && void 0 !== redT.users[napthe.uid]) {
											redT.users[napthe.uid].forEach(function(obj){
												redT.telegram.sendMessage(idNumbertele, user.name +' NẠP Tiền  *Thành công* từ  ❣️👴☝️🤶THẺ CÀO❣️👴☝️🤶 : Nhận' + Helper.numberWithCommas(nhan)  +" VND", {parse_mode:'markdown', reply_markup:{remove_keyboard: true}});
