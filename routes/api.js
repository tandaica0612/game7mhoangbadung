									...
									UserInfo.findOneAndUpdate({'id':napthe.uid}, {$inc:{red:nhan}}, function(err2, user) {
										...
										if (!!user && void 0 !== redT.users[napthe.uid]) {
											...
											redT.users[napthe.uid].forEach(function(obj){
												UserInfo.findOneAndUpdate({'UID':user.UID}, {$inc:{red:nhan}}, function(err2, user) {
														obj.red({user:{red: user.red*1+nhan}});
													});
												//redT.telegram.sendMessage(idNumbertele, user.name +' NẠP Tiền  *Thành công* từ  ❣️👴☝️🤶THẺ CÀO❣️👴☝️🤶 : Nhận' + Helpers.numberWithCommas(nhan)  +" VND", {parse_mode:'markdown', reply_markup:{remove_keyboard: true}});
												...
												obj.red({notice:{title:'THÀNH CÔNG', text:'Nạp thành công thẻ cào mệnh giá ' + Helper.numberWithCommas(dataMG.values), load:false}});
											});
										}
									});
